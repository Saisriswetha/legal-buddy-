import { jsx as _jsx, jsxs as _jsxs } from "react/jsx-runtime";
import { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { authService } from '../../services/auth';
import { LoadingSpinner } from '../Common/LoadingSpinner';
export const GoogleLoginButton = () => {
    const navigate = useNavigate();
    const [loading, setLoading] = useState(false);
    useEffect(() => {
        const urlParams = new URLSearchParams(window.location.search);
        const authenticated = urlParams.get('authenticated');
        const code = urlParams.get('code');
        console.log('Authenticated param:', authenticated);
        console.log('Code param:', code);
        if (authenticated === 'true') {
            console.log('Authenticated! Fetching current user...');
            setLoading(true);
            authService.getCurrentUser()
                .then((user) => {
                if (user) {
                    console.log('User fetched:', user);
                    authService.setStoredUser(user);
                    window.history.replaceState({}, document.title, window.location.pathname);
                    navigate('/chat');
                }
                else {
                    console.error('Failed to fetch user');
                    setLoading(false);
                }
            })
                .catch((error) => {
                console.error('Failed to fetch user:', error);
                setLoading(false);
            });
            return;
        }
        if (code) {
            console.log('Processing code:', code);
            setLoading(true);
            authService.loginWithGoogle(code)
                .then((response) => {
                console.log('Login successful:', response);
                authService.setStoredUser(response.user);
                navigate('/chat');
            })
                .catch((error) => {
                console.error('Login failed:', error);
                setLoading(false);
            });
        }
    }, [navigate]);
    const handleLogin = () => {
        window.location.href = 'http://localhost:8000/api/auth/google';
    };
    if (loading) {
        return _jsx(LoadingSpinner, { size: "lg" });
    }
    return (_jsxs("button", { onClick: handleLogin, className: "bg-white border-2 border-gray-300 text-gray-700 font-semibold py-3 px-6 rounded-lg hover:bg-gray-50 transition-colors flex items-center gap-2", children: [_jsxs("svg", { className: "w-5 h-5", viewBox: "0 0 24 24", children: [_jsx("path", { fill: "currentColor", d: "M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" }), _jsx("path", { fill: "currentColor", d: "M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" }), _jsx("path", { fill: "currentColor", d: "M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" }), _jsx("path", { fill: "currentColor", d: "M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" })] }), "Sign in with Google"] }));
};
